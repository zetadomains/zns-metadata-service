...
export async function ensMetadata(req: Request, res: Response) {
  ...
  try {
    ...
    try {
      // Here is the case; if subgraph did not index fresh ZNS name but registry has the record,
      ...
      assert(isRecordExist, 'ZNS name does not exist');
